...
def train(gloal_iter):
    ...
    i = 0
    parallel_train_loader = pl.MpDeviceLoader(train_loader, device)
    for batch in parallel_train_loader:
...
def evaluate(gloal_iter):
    ...
    parallel_val_loader = pl.MpDeviceLoader(val_loader, device)
    lossname_2_id = {}
    for i, name in enumerate(loss_name_list):
        lossname_2_id[name] = i
    ...
    for i, batch in enumerate(tqdm(parallel_val_loader, total=len(parallel_val_loader))):
        s_pose_loss, \
        c_motion_prd, c_motion_gt, c_motion_loss, c_motion_loss_mask, \
        v_motion_prd, v_motion_gt, v_motion_loss, \
        last_cx_prd, last_cx_gt, last_cx_loss = forward(model, batch)
...
if __name__ == "__main__":
    xmp.spawn(mp_fn, args=(...), nprocs=8, start_method='fork')