...
def forward(model, batch, last_clip_criterion):
...
def train(gloal_iter, model, train_loader, optimizer, device, cfg, last_clip_criterion, optim_scheduler, writer, val_loader):
    ...
    for batch in train_loader:
        ...
        c_motion_prd, c_motion_gt, c_motion_loss, c_motion_loss_mask, \
        v_motion_prd, v_motion_gt, v_motion_loss, \
        last_cx_prd, last_cx_gt, last_cx_loss = forward(model, batch, last_clip_criterion)
        ...
        if i % cfg.accum_iter == 0:
            ...
            if gloal_iter % cfg.eval_step == 0:
                with torch.no_grad():
                    epoch_val_log = evaluate(gloal_iter, model, val_loader, optimizer, device, cfg, writer)
...
def evaluate(gloal_iter, model, val_loader, optimizer, device, cfg, writer):
    model.eval()
...
def mp_fn(index, cfg):
 # *** load config
    ...
    while 1:
    # print("\ntraining.............")
        gloal_iter = train(gloal_iter, model, train_loader, optimizer, device, cfg, last_clip_criterion, optim_scheduler, writer, val_loader)  # [s_pose, t_pose, loss]
